# src/vyom/runner.py
"""
Runner: compile or interpret a Vyom source file, attempt VM first then fallback.

Usage:
    python -m vyom.runner path/to/file.vyom
"""
...
def read_source(path: str) -> str:
    """Read source file text, ensuring .vyom extension is handled by caller."""
    p = pathlib.Path(path)
    if not p.exists():
...
def main(argv: List[str] | None = None) -> int:
    ...
    if not argv:
        print("Usage: python -m vyom.runner path/to/file.vyom")
        return 1
    path = argv[0]
    # Enforce .vyom extension – only Vyom source files are allowed
    if pathlib.Path(path).suffix.lower() != ".vyom":
        print(f"Error: Only .vyom files are supported. Received '{path}'.")
        return 2
