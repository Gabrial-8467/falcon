"""
REPL for the Vyom language (JS-like).

Provides:
- start_repl() entry point
- multiline input support (basic brace/paren matching)
- readline history (saved to ~/.vyom_history) (falls back if readline missing)
...
def start_repl() -> None:
    """
    Start the Vyom interactive REPL. Blocks until the user exits.
    """
    interpreter = Interpreter()
    _setup_readline()
    print("Vyom REPL — type 'help' for commands, Ctrl-D to exit")
    source_lines: List[str] = []

    try:
        while True:
            try:
                prompt = "... " if source_lines else "vyom> "
                line = input(prompt)
            except EOFError:
                print()
                break
            ...
            if not source_lines and stripped == "help":
                ...
                print("  .load <file>         load and run a .vyom file")
...
# ✅ REQUIRED ENTRY POINT FOR `python -m vyom.repl`
def main() -> None:
    start_repl()
