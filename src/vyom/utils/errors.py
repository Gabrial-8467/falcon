"""
Custom error classes and helpers for the Vyom language.

These errors are *not* the same as Python's LexerError / ParseError / InterpreterError.
They provide a unified VyomError base class + optional pretty
...
class VyomError(Exception):
    """Base error for all Vyom-related issues."""
    pass
...
def format_error_message(msg: str, ctx: Optional[SourceContext]) -> str:
    ...
    (Vyom's runner/repl may choose to use this.)
    """
    if ctx is None:
        return msg
