...
def run_example(example_name: str):
    """Helper to run an example .vyom file and capture stdout."""
...
def test_hello_example():
    rc, output = run_example("hello.vyom")
    assert rc == 0
    assert "Hello" in output or output.strip() != ""


def test_factorial_example():
    rc, output = run_example("factorial.vyom")
    assert rc == 0
...
def test_closure_example():
    rc, output = run_example("closure.vyom")
    assert rc == 0
...
def test_async_stub_example():
    rc, output = run_example("async_stub.vyom")
    assert rc == 0
